...
function displayWelcomeMessage() {
    ...
    const welcomeMessage = {
        role: 'assistant',
        content: `${greeting} I'm Tree of Life AI. I provide health guidance from 8 medical traditions: Western Medicine, Ayurveda, TCM, Herbal Medicine, Homeopathy, Chiropractic, Clinical Nutrition, and Vibrational Healing. ${authToken ? 'What health topic can I help you explore?' : 'Please log in to start a conversation.'}`,
        timestamp: new Date().toISOString()
    };
    
    // Only show if it's a new conversation (check localStorage, not currentConversationId)
...
async function loadCurrentConversation() {
    ...
    try {
        ...
        // 🔧 FIX: Clear chat window BEFORE loading messages (prevents welcome message from showing)
