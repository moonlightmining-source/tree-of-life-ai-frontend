...
📷 **NEW:** You can now upload images! Share photos of herbs, meals, symptoms, or medical documents for analysis.
...
function handleImageSelect(event) {
    ...
    if (file.size > 5 * 1024 * 1024) {
        alert('⚠️ Image size must be less than 5MB. Please select a smaller image.');
    ...
    if (!supportedFormats.includes(file.type)) {
        alert('⚠️ Supported formats: JPG, PNG, GIF, WebP');
        event.target.value = '';
        return;
    }
    ...
    reader.onerror = function(error) {
        ...
        alert('⚠️ Failed to read image file. Please try again.');
        event.target.value = '';
    };
